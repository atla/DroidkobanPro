package de.leihwelt.android.astar;
...
public class AStarPathFinder implements PathFinder {
	...
	private void initNodes(TileBasedMap map) {

		Log.v("creating nodes...", " with " + map.getWidthInTiles() + " " + map.getHeightInTiles());
	...
	public Path findPath(Mover mover, int sx, int sy, int tx, int ty) {
		...
		if (tx >= 0 && tx < map.getWidthInTiles() && ty >= 0 && ty < map.getHeightInTiles()) {
			...
			while ((maxDepth < maxSearchDistance) && (open.size() != 0)) {
				...
				for (int x = -1; x < 2; x++) {
					for (int y = -1; y < 2; y++) {
						...
						// determine the location of the neighbour and evaluate
						// it
						...
						if (isValidLocation(mover, sx, sy, xp, yp, tx, ty)) {
							// the cost to get to this node is cost the current
							// plus
							// the movement
							...
							// it has been previously makes sure the node
							// hasn'e've
							// determined that there might have been a better
							// path
							// to get to
							...
							// reset it's cost to our current cost and add it as
							// a
							// next possible
			...
			// At this point we've definitely found a path so we can uses the
			// parent

			// references of the nodes to find out way from the target location
			// back
	 ...
	 */
	protected boolean isValidLocation(Mover mover, int sx, int sy, int x, int y, int dstX, int dstY) {
		boolean invalid = (x < 0) || (y < 0) || (x >= map.getWidthInTiles()) || (y >= map.getHeightInTiles());

		if ((!invalid) && ((sx != x) || (sy != y))) {

			if (x == dstX && y == dstY)
				invalid = map.isWall(mover, x, y);
			else
	...
	@Override
	public void init(TileBasedMap map) {
		initNodes(map);
