...
public class Font2D {
	...
	public Font2D(GL10 gl, String fontName, int size, FontStyle style) {
	...
	public Font2D(GL10 gl, AssetManager assets, String file, int size, FontStyle style) {
		this.texture = new Texture(gl, 256, 256, TextureFilter.Nearest, TextureFilter.Nearest, TextureWrap.ClampToEdge, TextureWrap.ClampToEdge);
	...
	private Glyph createGlyph(char character) {
		...
		Glyph glyph = new Glyph(getGlyphAdvance(character), (int) rect.width, (int) rect.height, glyphX / 256.0f, glyphY / 256.0f, rect.width / 256.0f,
				rect.height / 256.0f);
		glyphX += rect.width;
		return glyph;
	}
	...
	public class Text {
		...
		private void rebuild() {
			if (mesh == null)
				mesh = new GLMesh(gl, 6 * text.length(), false, true, false);
