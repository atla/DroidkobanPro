...
public final class GLMesh {
	...
	private static GLMesh storedMesh;

	public static boolean activeVBO = true;

	public static int meshCount = 0;

	public GLMesh(GL10 gl, int numVertices, boolean hasColors, boolean hasTextureCoordinates, boolean hasNormals) {
		this.gls = gl;
	 ...
	 */
	private int getPrimitiveType(GLPrimitiveType type) {
		if (type == GLPrimitiveType.Lines)
			return GL10.GL_LINES;
		else if (type == GLPrimitiveType.Triangles)
			return GL10.GL_TRIANGLES;
		else if (type == GLPrimitiveType.LineStrip)
			return GL10.GL_LINE_STRIP;
		else if (type == GLPrimitiveType.TriangleStrip)
			return GL10.GL_TRIANGLE_STRIP;
		else if (type == GLPrimitiveType.Points)
	 ...
	 */
	public void render(GLPrimitiveType type, int offset, int numVertices) {
		boolean wasDirty = dirty;
		if (dirty)
			update();

		if (this == storedMesh && !wasDirty) {
			gls.glDrawArrays(getPrimitiveType(type), offset, numVertices);
			return;
		} else {
			gls.glDisableClientState(GL10.GL_VERTEX_ARRAY);
			gls.glDisableClientState(GL10.GL_COLOR_ARRAY);
			gls.glDisableClientState(GL10.GL_TEXTURE_COORD_ARRAY);
			gls.glDisableClientState(GL10.GL_NORMAL_ARRAY);
		}

		gls.glEnableClientState(GL10.GL_VERTEX_ARRAY);
		if (activeVBO) {
			((GL11) gls).glBindBuffer(GL11.GL_ARRAY_BUFFER, vertexHandle);
			((GL11) gls).glVertexPointer(3, GL10.GL_FLOAT, 0, 0);
		} else
			gls.glVertexPointer(3, GL10.GL_FLOAT, 0, vertexBuffer);

		if (colors != null) {
			gls.glEnableClientState(GL10.GL_COLOR_ARRAY);
			if (activeVBO) {
				((GL11) gls).glBindBuffer(GL11.GL_ARRAY_BUFFER, colorHandle);
				((GL11) gls).glColorPointer(4, GL10.GL_FLOAT, 0, 0);
			} else
				gls.glColorPointer(4, GL10.GL_FLOAT, 0, colorBuffer);
		}

		if (texCoords != null) {
			gls.glEnableClientState(GL10.GL_TEXTURE_COORD_ARRAY);
			if (activeVBO) {
				((GL11) gls).glBindBuffer(GL11.GL_ARRAY_BUFFER, texHandle);
				((GL11) gls).glTexCoordPointer(2, GL10.GL_FLOAT, 0, 0);
			} else
				gls.glTexCoordPointer(2, GL10.GL_FLOAT, 0, texCoordBuffer);
		}

		if (normals != null) {
			gls.glEnableClientState(GL10.GL_NORMAL_ARRAY);
			if (activeVBO) {
				((GL11) gls).glBindBuffer(GL11.GL_ARRAY_BUFFER, normalHandle);
				((GL11) gls).glNormalPointer(GL10.GL_FLOAT, 0, 0);
			} else
				gls.glNormalPointer(GL10.GL_FLOAT, 0, normalBuffer);
		}
	 ...
	 *            the type
	 */
	public void render(GLPrimitiveType type) {
